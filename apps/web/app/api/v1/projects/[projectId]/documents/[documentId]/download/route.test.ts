...
describe("GET /api/v1/projects/{projectId}/documents/{documentId}/download", () => {
  beforeEach(() => {
  ...
  it("VIEWER도 서명된 다운로드 URL을 받을 수 있다", async () => {
    getAuthenticatedUser.mockResolvedValue({ id: "user-1" });
    withRequestScope.mockImplementation(async (_scope, fn) =>
      fn({ query: vi.fn().mockResolvedValue({ rows: [documentRow] }) } as never)
    );
    getMembership.mockResolvedValue({ role: "VIEWER" });
    createPresignedDownloadUrl.mockResolvedValue("https://storage.example.com/signed-url");
    const { GET } = await import("./route");
    ...
    expect(body.data.url).toBe("https://storage.example.com/signed-url");
    expect(body.data.filename).toBe("감사보고서.pdf");
    expect(createPresignedDownloadUrl).toHaveBeenCalledWith("org-1/project-1/doc-1/report.pdf");
  });
});
