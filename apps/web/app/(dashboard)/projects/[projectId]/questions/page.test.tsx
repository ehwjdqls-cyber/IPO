...
const { getAuthenticatedUser, findProjectById, getMembership, listQuestionsWithKpi, notFound } =
  vi.hoisted(() => ({
...
describe("S10 Q&A 목록", () => {
  ...
  it("질문 텍스트가 워크스페이스로 가는 링크다", async () => {
    getMembership.mockResolvedValue({ role: "VIEWER" });
    listQuestionsWithKpi.mockResolvedValue({
      kpi: emptyKpi,
      questions: [
    ...
    const link = findNode(
      result,
      (n) => n.type === Link && String(n.props?.href ?? "").includes("/questions/question-1")
    );
    expect(link?.props?.href).toBe("/projects/project-1/questions/question-1");
    expect(collectText(link)).toBe("2025년 매출액은 얼마입니까?");
  });
